...
const Offers = () => {
    ...
    const handleDeleteWishlist = async (id) => {
        setIsLoading(true)
        try {
            await deleteWishList(id, accessToken)
            setRefetch(!refetch)
        } catch (error) {
            console.log(error)
        } finally {
    ...
    return (
        <div className="container mt-11">
            ...
            <h1 className='text-[30px] font-bold font-primary uppercase text-dark'>{t("offers")}</h1>
            <div>
                {isLoading && (
                                ...
                                return (
                                    <div key={product.id} className="p-4 shadow-main rounded-md border-[1px] border-slate-200">
                                        ...
                                        >
                                            ...
                                            />
                                            <img src={product.product.images[0].image_url} className='w-full aspect-square' alt="" />
                                        </div>
